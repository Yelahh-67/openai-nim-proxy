...
app.listen(PORT, () => console.log(`Server live on ${PORT}`));      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
...
app.listen(PORT, () => console.log(`Server live on ${PORT}`));/ server.js - OpenAI to NVIDIA NIM API Proxy
const express = require('express');
const cors = require('cors');
const axios = require('axios');
...
// 🔥 REASONING DISPLAY TOGGLE - Menampilkan/menyembunyikan tag <think> di output akhir
const SHOW_REASONING = true; 

// 🔥 THINKING MODE TOGGLE - Mengaktifkan parameter 'enable_thinking' untuk model yang mendukung (seperti GLM-4.7)
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    let nimModel = MODEL_MAPPING[model];
    if (!nimModel) {
      // Logic fallback tetap sama seperti script asli Anda
      nimModel = model; 
    }
    
    // --- UPDATE DI SINI ---
    // Transform OpenAI request to NIM format dengan 'enable_thinking'
    const nimRequest = {
      model: nimModel,
      messages: messages,
      temperature: temperature || 0.6,
      max_tokens: max_tokens || 9024,
      // Menggunakan enable_thinking: true sesuai standar GLM-4.7/vLLM
    ...
    if (stream) {
      ...
      // Transform NIM response ke format OpenAI (Non-Stream)
      const openaiResponse = {
        id: `chatcmpl-${Date.now()}`,
        object: 'chat.completion',
        created: Math.floor(Date.now() / 1000),
...
app.listen(PORT, () => {
  ...
  console.log(`Thinking mode is ${ENABLE_THINKING_MODE ? 'ON' : 'OFF'}`);
});    status: 'ok', 
    service: 'OpenAI to NVIDIA NIM Proxy', 
    reasoning_display: SHOW_REASONING,
    thinking_mode: ENABLE_THINKING_MODE
